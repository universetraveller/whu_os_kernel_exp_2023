...
#endif /* CONFIG_COMPAT */
SYSCALL_DEFINE0(common_printk_test){
        printk("KERNEL_DEBUG# Hello World!\n");
        return 0;
}

#include <linux/time.h>
SYSCALL_DEFINE0(print_time_kernel){
	struct timespec64 ts;
	struct tm m_tm;
	ktime_get_real_ts64(&ts);
	time64_to_tm(ts.tv_sec, 28800, &m_tm);
        printk("KERNEL_DEBUG# NOW_TIME IN BEIJING: %04d-%02d-%02d %02d:%02d:%02d\n",
...
#include <linux/rtc.h>
SYSCALL_DEFINE0(print_time_kernel_1){
	struct timespec64 ts;
	struct rtc_time m_rtc;
	ktime_get_real_ts64(&ts);
	rtc_time64_to_tm(ts.tv_sec, &m_rtc);
	// ETC +8 -> hour + 8
        printk("KERNEL_DEBUG# NOW_TIME IN BEIJING: %04d-%02d-%02d %02d:%02d:%02d\n",
           m_rtc.tm_year + 1900, m_rtc.tm_mon + 1, m_rtc.tm_mday,
           m_rtc.tm_hour+8, m_rtc.tm_min, m_rtc.tm_sec);
        return 0;
}

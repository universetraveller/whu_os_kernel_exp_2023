...
{
	printk(KERN_DEBUG"kernel.sched.core::context_switch# From %d@%d switch to %d@%d (pid@tgid)", prev->pid, prev->tgid, next->pid, next->tgid);
	printk(KERN_DEBUG"kernel.sched.core::context_switch# Regs info-> pre.sp<%x>, next.sp<%x>", prev->thread.sp, next->thread.sp);
...
__pick_next_task(struct rq *rq, struct task_struct *prev, struct rq_flags *rf)
{
	...
	if(p){
		//printk("kernel.sched.core::__pick_next_task# use stop_sched_class");
		return p;
	}else{
		p = dl_sched_class.pick_next_task(rq);
		if(p){
		//printk("kernel.sched.core::__pick_next_task# use dl_sched_class");
			return p;
		}else{
			p = rt_sched_class.pick_next_task(rq);
			if(p){
		//printk("kernel.sched.core::__pick_next_task# task %d use rt_sched_class", p->pid);
				return p;
			}else{
				p = fair_sched_class.pick_next_task(rq);
				if(p){
		//printk("kernel.sched.core::__pick_next_task# use fair_sched_class");
					return p;
				}else{
					p = idle_sched_class.pick_next_task(rq);
					if(p){
		//printk("kernel.sched.core::__pick_next_task# use idle_sched_class");
						return p;
					}
				}
			}
	...
	*/
	...
	printk("kernel.sched.core::__pick_next_task# Unexpected reach unreachable statement");
	BUG(); /* The idle class should always have a runnable task. */
}
static inline struct task_struct *
__pick_next_task_ori(struct rq *rq, struct task_struct *prev, struct rq_flags *rf)
{
	const struct sched_class *class;
	struct task_struct *p;
...
static void __sched notrace __schedule(unsigned int sched_mode)
{
	...
	//printk(KERN_DEBUG"kernel.sched.core::__schedule# next task is %d@%d", next->pid, next->tgid);

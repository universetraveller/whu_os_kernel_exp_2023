...
{
	/*
	void* test_p = kmap(page);
	if(test_p){
		printk("page_alloc.c, 2422: free_unref_page_commit -> %x", (int*) test_p);
	}else{
		printk("page_alloc.c, 2422: free_unref_page_commit -> <unk>", (int*) test_p);
	}
	*/
	int high;
	int pindex;
...
void free_unref_page(struct page *page, unsigned int order)
{
	...
	void* test_p = page_address(page);
	if(test_p){
		printk(KERN_DEBUG"page_alloc.c, 2470: free_unref_page -> %x", (int*) test_p);
	}else{
		printk(KERN_DEBUG"page_alloc.c, 2470: free_unref_page -> <unk>", (int*) test_p);
	}
...
void free_unref_page_list(struct list_head *list)
{
	...
	int batch_count = 0;
	int migratetype;
	printk(KERN_DEBUG"page_alloc.c, 2513: free_unref_page_list");

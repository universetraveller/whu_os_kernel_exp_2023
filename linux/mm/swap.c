...
void folio_mark_accessed(struct folio *folio)
{
	...
	if(test_p){
		printk(KERN_DEBUG"%x is accessed", (int*) test_p);
	}
	
	// code for multi-generational lru, assuming not needy
	/*
	if (lru_gen_enabled()) {
	...
	*/
	...
	if (!folio_test_referenced(folio)) {
		// not referenced, should keep to prevent the folio goes to active lru
		//folio_set_referenced(folio);
		...
		// do not allow folio gets active to implement FIFO
		/*
		...
		*/	
		...
		// func to mark folio is active, should not be used
